...
for inp in p.glob('luk*.mp4'):
    ...
    if len(os.listdir(dir_out)) == 0: # Check is empty..
        os.system(f"python demo.py mot --load_model ../models/mcFairmotPose_without-val.pth --conf_thres 0.2 --det_thres 0.7 --emb_sim_thres 0.1 --iou_sim_thres 0.1 --input_video {str(inp)} --output_root {str(dir_out)} --output_format text")
    else:
        print('not this one==============================')